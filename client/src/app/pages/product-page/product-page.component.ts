...
//   if (!searchText) {
//     this.filteredProducts = [...this.products];  // Если текст пустой, показываем все товары
//   } else {
//     this.filteredProducts = this.products.filter(product => 
//       product.name.toLowerCase().includes(searchText.toLowerCase())  // Фильтруем по имени товара
//     );
//   }
// }
...
export class ProductPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.filterTextService.currentText$.subscribe((text) => {
      this.selectedText = text;
      this.filterProductsByCategory(this.selectedText);
    });
  }
...
filterProductsByCategory(category: string) {
  if (category === 'Все') {
    ...
    this.filteredProducts = this.products.filter(product => product.category === category);  // Фильтруем по категории
  }
}
