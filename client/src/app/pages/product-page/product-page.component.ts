...
export class ProductPageComponent implements OnInit {
  ...
  selectedText: string = 'Все';  // Категория, по умолчанию "Все"
  ...
  products: Product[] = [
    ...
    { 
      name: 'Оперативная память Adata XPG', 
      price: 7490,  
      rating: 4.7, 
      warranty: 36, 
      manufacturer: 'Adata', 
  ...
  ngOnInit(): void {
    this.filterTextService.currentText$.subscribe((text) => {
      ...
      this.filterProducts();  // Фильтруем при изменении категории
    });
  }
  ...
  filterProducts() {
    this.filteredProducts = this.products.filter(product => {
      ...
      const isMatchingCategory = this.selectedText !== 'Все' ? product.category === this.selectedText : true;

      return isAboveMinPrice && isBelowMaxPrice && isAboveMinWarranty && isBelowMaxWarranty && isMatchingManufacturer && isMatchingCategory;
    });
  }
  ...
  // Метод для фильтрации продуктов по введенному тексту
  onSearchTextChanged(searchText: string) {
    if (!searchText) {
      this.filteredProducts = [...this.products];  // Если текст пустой, показываем все продукты
    } else {
      this.filteredProducts = this.products.filter(product =>
  ...
  // Метод для сброса фильтров и состояния
  onResetFilters() {
    this.selectedText = 'Все';  // Устанавливаем категорию на "Все"
    this.minPrice = null;
    this.maxPrice = null;
    this.minWarranty = null;
    this.maxWarranty = null;
    this.selectedManufacturer = null;  // Очищаем производителя
    this.searchText = '';  // Очищаем текст поиска

    this.filterProducts();  // Применяем сброс фильтров
  }
}
